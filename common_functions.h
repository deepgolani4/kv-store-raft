...
int ring_hash(const string &s) {
  int j = RING_SIZE;
  int h = 0;
  for (char c : s)
    h = (h + ((int)c * HASH_MUL) % j) % j;
  return (h + j) % j;
}

int make_tcp_server(const string &ip, const string &port) {
...
void tcp_connect(int fd, const string &ip, const string &port) {
  sockaddr_in addr{};
  addr.sin_family = AF_INET;
  addr.sin_addr.s_addr = inet_addr(ip.c_str());
  addr.sin_port = htons(stoi(port));
...
void net_send(int fd, const string &msg) {
  char buf[BUFF_SIZE];
...
string net_recv(int fd) {
  char buf[BUFF_SIZE] = {0};
  recv(fd, buf, BUFF_SIZE, 0);
  return string(buf);
}

pair<string, string> split_addr(const string &addr) {
